...
import { ArrowDown, ArrowUp, Minus, TrendingUp, Sparkles, RefreshCw, Radar, Info, ShieldAlert } from "lucide-react";
...
import { competitors as seedCompetitors, competitorPrices as seedCompetitorPrices, pricingItems as seedPricingItems } from "@/lib/mock-data";
...
function CompetitorsPage() {
  ...
  const [activeCompareCompetitor, setActiveCompareCompetitor] = useState<any | null>(null);
  const [isScraping, setIsScraping] = useState(false);

  const [competitors, setCompetitors] = useState<any[]>(seedCompetitors);
  const [competitorPrices, setCompetitorPrices] = useState<any[]>(seedCompetitorPrices);
  ...
  const activeProduct = pricingItems.find((p) => p.id === selectedProductId) ?? pricingItems[0];
  ...
  const simulatedCompetitors = competitors.map((c, i) => {
    ...
    if (c.id === "c1" && activeProduct) itemPrice = Math.round(activeProduct.competitorAvg * 1.03); // GlobalLogix tends to be highest
    if (c.id === "c2" && activeProduct) itemPrice = activeProduct.competitorAvg; // Nexus is avg
    if (c.id === "c3" && activeProduct) itemPrice = Math.round(activeProduct.competitorAvg * 0.98); // Apex is slightly lower
    if (c.id === "c4" && activeProduct) itemPrice = Math.round(activeProduct.competitorAvg * 0.95); // Meridian is cheapest
      ...
      <div className="p-6 lg:p-8 max-w-[1600px] mx-auto">
        <PageHeader
          ...
          actions={
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={triggerScrapeRefresh} disabled={isScraping}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <KpiCard
            ...
            accent="warning"
          />
          <KpiCard label="Pricing Opportunities" value="9 SKUs" delta="+3 new" trend="up" accent="primary" />
          <KpiCard label="Significant Moves (24h)" value="14 matches" hint="Changes exceeding ±3.5%" />
        </div>
        ...
        <div className="flex items-center gap-3 mb-6 bg-card border border-border p-4 rounded-lg">
          ...
          <Select value={selectedProductId} onValueChange={setSelectedProductId}>
            <SelectTrigger className="w-72 bg-surface-2"><SelectValue /></SelectTrigger>
            <SelectContent>
              {pricingItems.map((p) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
          {simulatedCompetitors.map((c) => (
            <Card key={c.id} className="p-5 gap-3 flex flex-col justify-between">
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-semibold text-sm">{c.name}</p>
                  <p className="text-[10px] text-muted-foreground">{c.monitored} SKUs tracked</p>
                </div>
                <Badge variant="outline" className="font-mono text-xs">{c.marketShare}% share</Badge>
              </div>
              <div className="my-2">
                <span className="text-2xl font-bold font-mono tracking-tight">${c.itemPrice.toLocaleString()}</span>
                <span className="text-[10px] text-muted-foreground ml-1.5">for {activeProduct.product}</span>
              </div>
              <div className="flex items-center justify-between border-t border-border/55 pt-3 mt-1">
                <TrendBadge t={c.trend} />
                <Badge variant="outline" className={c.availability === "In Stock" ? "bg-success/10 text-success border-success/30 text-[10px]" : "bg-destructive/10 text-destructive border-destructive/30 text-[10px]"}>
                  {c.availability}
                </Badge>
              </div>
            </Card>
        ...
        {/* Comparison Table */}
        <SectionCard title="Product Comparative Price Matrix" description={`Competitor pricing audit matrix for ${activeProduct.product}`}>
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {simulatedCompetitors.map((c) => {
                ...
                return (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-right font-mono font-semibold">${c.itemPrice.toLocaleString()}</TableCell>
                    <TableCell className={`text-right font-mono ${diff > 0 ? "text-success" : diff < 0 ? "text-destructive" : "text-muted-foreground"}`}>
                      {diff > 0 ? "+" : ""}
                      {diffPct.toFixed(1)}%
                    </TableCell>
                    <TableCell><TrendBadge t={c.trend} /></TableCell>
                    <TableCell>
                      <Badge variant="outline" className={c.availability === "In Stock" ? "bg-success/10 text-success border-success/30" : "bg-destructive/10 text-destructive border-destructive/30"}>
                        {c.availability}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="outline" size="sm" onClick={() => setActiveCompareCompetitor(c)}>Compare</Button>
                    </TableCell>
                  </TableRow>
                );
              })}
        ...
        <Dialog open={showOppDialog} onOpenChange={setShowOppDialog}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                The pricing crawler detected SokoPulse prices lagging behind competitor averages. Approved adjustments can boost margins.
              </DialogDescription>
            </DialogHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">SokoPulse Price</TableHead>
                    <TableHead className="text-right">Competitor Avg</TableHead>
                    <TableHead className="text-right">Potential Margin Gain</TableHead>
                ...
                <TableBody>
                  {pricingItems.slice(0, 3).map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right font-mono text-xs">${item.currentPrice}</TableCell>
                      <TableCell className="text-right font-mono text-xs">${item.competitorAvg}</TableCell>
                      <TableCell className="text-right text-success font-mono text-xs font-semibold">
                        +${(item.recommendedPrice - item.currentPrice).toFixed(0)} ({item.expectedImpact}%)
                      </TableCell>
                      <TableCell className="text-right">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowOppDialog(false)}>Close</Button>
              <Button onClick={() => {
        ...
        <Dialog open={!!activeCompareCompetitor} onOpenChange={(o) => !o && setActiveCompareCompetitor(null)}>
          <DialogContent className="sm:max-w-[450px]">
            {activeCompareCompetitor && (
              <>
                <DialogHeader>
                  <DialogTitle>Competitor Audit: {activeCompareCompetitor.name}</DialogTitle>
                  <DialogDescription>
                ...
                <div className="space-y-4 py-3">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="p-3 bg-surface-2 border border-border rounded-md">
                      <span className="text-muted-foreground uppercase text-[10px]">Scrape Status</span>
                      <p className="font-semibold mt-1 text-success flex items-center gap-1">
                        <span className="size-2 rounded-full bg-success inline-block" /> Active
                      </p>
                    </div>
                    <div className="p-3 bg-surface-2 border border-border rounded-md">
                      <span className="text-muted-foreground uppercase text-[10px]">Scrape Target Frequency</span>
                      <p className="font-semibold mt-1">4 Hours Cadence</p>
                    </div>
                    <div className="p-3 bg-surface-2 border border-border rounded-md">
                      <span className="text-muted-foreground uppercase text-[10px]">Scraped Endpoints</span>
                      <p className="font-semibold mt-1">{activeCompareCompetitor.monitored} active pages</p>
                    </div>
                    <div className="p-3 bg-surface-2 border border-border rounded-md">
                      <span className="text-muted-foreground uppercase text-[10px]">Scrape Success Rate</span>
                  ...
                  <div className="rounded-lg bg-primary/5 border border-primary/20 p-3 text-xs leading-relaxed text-muted-foreground">
                    ...
                    {activeCompareCompetitor.id === "c1" ? (
                      <span>GlobalLogix operates as a premium supplier with longer delivery windows. They lead prices upwards, creating high margin adjustment buffers.</span>
                    ) : activeCompareCompetitor.id === "c4" ? (
                      <span>Meridian Imports operates as a discount distributor. They trade margins for volume. We recommend keeping pricing alerts tight here.</span>
                    ) : (
                      <span>Nexus Pro maintains a matching index with SokoPulse. They actively scrap SokoPulse catalog and maintain exact parity within ±2.5%.</span>
                    )}
                  </div>
                </div>

                <DialogFooter>
                  <Button onClick={() => setActiveCompareCompetitor(null)}>Close Audit</Button>
                </DialogFooter>
              </>
            )}
          </DialogContent>
...
function TrendBadge({ t }: { t: "up" | "down" | "flat" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded font-mono ${cls}`}>
