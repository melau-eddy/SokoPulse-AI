...
function AlertsPage() {
  ...
  const handleResolve = (id: string, currentlyResolved: boolean) => {
    ...
    apiClient.resolveAlert(id, nextResolvedState).then((serverAlert) => {
      setData((prev) =>
        prev.map((item) =>
          item.id === id ? { ...item, resolved: nextResolvedState } : item,
        ),
      );
      toast.success(
        currentlyResolved
          ? "Alert marked as unresolved"
  ...
  const handleResolveAll = () => {
    ...
    const resolvePromises = unresolved.map((item) =>
      apiClient.resolveAlert(item.id, true),
    );
    ...
    Promise.all(resolvePromises).then(() => {
      ...
      toast.success(
        `Resolved ${unresolved.length} alerts in current filter view.`,
      );
      if (typeof window !== "undefined") {
        window.dispatchEvent(new Event("alerts-updated"));
      }
      ...
      <SectionCard>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-4 border-b border-border/40 pb-4">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <Select value={categoryFilter} onValueChange={setCategoryFilter}>
              <SelectTrigger className="w-full md:w-44 bg-surface-2">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-3">
          {list.map((a) => (
            <Card
              key={a.id}
              className={`p-4 flex flex-row items-start gap-4 transition-all ${a.resolved ? "opacity-45 bg-muted/20" : "bg-card hover:border-primary/20"}`}
            >
              <div className="flex-1 min-w-0">
                ...
                <p
                  className={`font-semibold text-sm ${a.resolved ? "line-through text-muted-foreground" : "text-foreground"}`}
                >
                  {a.title}
                </p>
          ...
          {list.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                No alerts match your current filter preferences.
              </p>
            </div>
          )}
        </div>
