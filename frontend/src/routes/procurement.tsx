...
import { Sparkles, ArrowRight, Truck, Clock, DollarSign, Calendar, Plus, RefreshCw, X } from "lucide-react";
...
import { fmtCurrency, procurementItems as seedItems, type ProcurementItem } from "@/lib/mock-data";
import { apiClient } from "../lib/api-client";
import { toast } from "sonner";

export const Route = createFileRoute("/procurement")({
  head: () => ({ meta: [{ title: "Procurement Recommendations — SokoPulse AI" }] }),
...
function ProcurementPage() {
  ...
  const [activePOs, setActivePOs] = useState<StatefulProcurement[]>(mockActivePOs);
  const [activeTab, setActiveTab] = useState<"pending" | "active">("pending");
  const [adjustItem, setAdjustItem] = useState<StatefulProcurement | null>(null);
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto">
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button
              ...
              size="sm"
              onClick={() => {
                setRecommendations(seedItems.map((r) => ({ ...r, status: "pending" })));
            ...
            <Button size="sm" onClick={handleBulkApprove} disabled={isBulkApproving || criticalCount === 0}>
              <RefreshCw className={`size-3.5 mr-1.5 ${isBulkApproving ? "animate-spin" : ""}`} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <KpiCard label="Recommended POs" value={`${pendingItems.length}`} accent="primary" />
        <KpiCard label="Total Budget Impact" value={fmtCurrency(totalBudgetImpact)} hint="Across all recommendations" />
        <KpiCard label="Critical Urgency" value={`${criticalCount}`} accent={criticalCount > 0 ? "destructive" : undefined} />
        <KpiCard label="Active POs In-Flight" value={`${activePOs.length}`} accent="success" />
      </div>

      <div className="mb-4">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
          <TabsList>
            <TabsTrigger value="pending">Recommendations ({pendingItems.length})</TabsTrigger>
            <TabsTrigger value="active">Active Orders ({activePOs.length})</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      ...
      {activeTab === "pending" ? (
        <div className="space-y-4">
            ...
            <Card key={p.id} className="p-6 gap-4">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">Supplier: {p.supplier} · Lead time: {p.leadTime} days</p>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="sm" className="h-9 text-destructive hover:bg-destructive/10" onClick={() => handleIgnore(p.id)}>
                    <X className="size-4" />
                  </Button>
                  <Button variant="outline" size="sm" className="h-9" onClick={() => openAdjustDialog(p)}>
                    Adjust
                  </Button>
                  <Button size="sm" className="h-9" onClick={() => handleCreatePO(p.id)}>
                    Create PO
                  </Button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <Stat label="Recommended Qty" value={`${p.qty.toLocaleString()} units`} />
              ...
              <div className="rounded-md border border-border bg-surface-2 p-3 flex gap-2">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  <span className="font-medium text-foreground">AI reasoning:</span> {p.reason}
                </p>
              </div>
            </Card>
          ))}
          {pendingItems.length === 0 && (
            <div className="text-center py-12 bg-card border border-border rounded-lg">
              <p className="font-medium text-sm text-muted-foreground">All recommendations resolved.</p>
            </div>
          )}
        </div>
      ) : (
        <div className="space-y-4">
          {activePOs.map((p) => (
            <Card key={p.id} className="p-6 gap-4 border-l-2 border-l-success">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono text-xs text-muted-foreground">{p.poNumber}</span>
                    <h3 className="font-semibold text-base">{p.product}</h3>
                    <Badge variant="outline" className="bg-success/10 text-success border-success/30 font-mono text-[10px]">
                      {p.trackingStatus}
                    </Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">Supplier: {p.supplier} · Handled via platform</p>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={() => toast.info(`Syncing status for PO ${p.poNumber}...`)}>
                    Track
                  </Button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                <Stat label="Ordered Qty" value={`${p.qty.toLocaleString()} units`} />
          ...
          {activePOs.length === 0 && (
            <div className="text-center py-12 bg-card border border-border rounded-lg">
              <p className="font-medium text-sm text-muted-foreground">No active purchase orders.</p>
            </div>
          )}
        </div>
      )}
          ...
          {adjustItem && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Modify quantity and supplier for your upcoming purchase order. Cost will recalculate.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Baseline recommendation: {adjustItem.qty.toLocaleString()} units
                  </p>
                </div>
                ...
                {parseInt(adjustQty) > 0 && (
                  <div className="rounded-lg bg-surface-2 border border-border p-3 space-y-2 text-xs">
                    <p className="font-semibold text-foreground">Live Budget Impact</p>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">New Quantity:</span>
                      <span className="font-mono">{parseInt(adjustQty).toLocaleString()} units</span>
                    </div>
                    <div className="flex justify-between">
