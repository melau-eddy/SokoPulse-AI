...
import { a as apiClient, P as PageHeader, b as Button } from "./router-RTlfstu9.js";
...
import { f as formatPrice } from "./mock-data-Nl4nWPc2.js";
...
function formatSeedRow(reportId, row) {
  if (reportId === "inv") {
    return {
      ...row,
...
function ReportsPage() {
  ...
  const [reportRows, setReportRows] = useState(() => activeReport.data.map((row) => formatSeedRow(activeReport.id, row)));
  useEffect(() => {
    setReportRows(activeReport.data.map((row) => formatSeedRow(activeReport.id, row)));
